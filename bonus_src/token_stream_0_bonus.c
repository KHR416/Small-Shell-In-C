...
/*   Updated: 2025/03/25 17:42:16 by wchoe            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	realloc_token_stream(t_token_stream *stream, size_t new_cap)
{
	...
	temp = ft_realloc(stream->arr, sizeof(t_token) * \
	stream->cap, sizeof(t_token) * new_cap);
	if (!temp)
		return (FAILURE);
	stream->arr = temp;
	stream->cap = new_cap;
...
{
	if (stream->cap < stream->len + 2 && \
		realloc_token_stream(stream, stream->cap << 1))
		return (FAILURE);
