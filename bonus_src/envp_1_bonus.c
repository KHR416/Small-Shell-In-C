...
/*   Updated: 2025/03/30 17:50:35 by chakim           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	add_env_variable(char ***envp, char *name, \
	char *value, t_buf *buf)
{
	size_t	envp_idx;
	void	*temp;
...
int	ms_setenv(char *name, char *value, char ***envp, t_buf *buf)
{
	...
	{
		env_name_len = 0;
		while ((*envp)[envp_idx][env_name_len] && \
		(*envp)[envp_idx][env_name_len] != '=')
			++env_name_len;
		if (!ft_memcmp(name, (*envp)[envp_idx], env_name_len))
			break ;
	}
	if (!(*envp)[envp_idx])
		return (add_env_variable(envp, name, value, buf));
	temp = create_new_env(name, value, buf);
	if (!temp)
		return (FAILURE);
	free((*envp)[envp_idx]);
